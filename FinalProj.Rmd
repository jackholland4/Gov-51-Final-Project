...
CT_votes <- merged_CT |>
  select(town, rep_vote_percent_diff, dem_vote_percent_diff, votedif)
...
finaldata <- finaldata |>
  ...
  mutate(dem_raw_vote_swing = round(dem_vote_percent_diff*Population)) |>
...
```{r}
##regression
damage_regression <- lm(dem_vote_percent_diff ~ Total.Damaged..any.type. + pct_white + Population + Per.capita.income + Median.household.income + coastal_binary, data = finaldata)
...
##subsetted plot
outlier_removed <- damaged_subset[damaged_subset$votedif >= -0.1, ]

damage_viz <- ggplot(data = outlier_removed, aes(x = Total.Damaged..any.type., y = votedif)) +
  geom_point(color = "blue") +
  geom_smooth(method = "lm", color = "blue") +
  labs(x = "Damage", y = "Dem Vote Percent Difference", title = "Property Damage vs. Democratic Vote Shift (Subsetted to Towns Damaged)") +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
damage_viz

##DID
damaged_votes <- mean(finaldata$dem_raw_vote_swing[finaldata$condition == "Damage"], na.rm = TRUE)
no_damage_vote <- mean(finaldata$dem_raw_vote_swing[finaldata$condition == "No Damage"], na.rm = TRUE)

dif_damage_numeric = ((damaged_votes - no_damage_vote)*10)
dif_damage_numeric
```
