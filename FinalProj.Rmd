...
```{r}
##regression
damage_regression <- lm(dem_vote_percent_diff ~ Total.Damaged..any.type. + pct_white + Population + Per.capita.income + Median.household.income, data = finaldata)
...
##DID
damaged_votes <- mean(finaldata$votedif[finaldata$condition == "Damage"], na.rm = TRUE)
no_damage_vote <- mean(finaldata$votedif[finaldata$condition == "No Damage"], na.rm = TRUE)

dif_damage_pct = ((damaged_votes - no_damage_vote)*10)
dif_damage_pct
```
