...
CT_votes$IsCoastal <- ifelse(CT_votes$town %in% coastal_towns, "Coastal", "Non-Coastal")
```

```{r}
ggplot(CT_votes, aes(x = rep_vote_percent_diff, y = dem_vote_percent_diff, label = town)) +
  geom_point(aes(color = IsCoastal, alpha = IsCoastal), size = 3) +
  scale_color_manual(values = c("Coastal" = "blue", "Non-Coastal" = "grey")) +
  xlim(-1.3, 0.9) +  # Set x-axis limits
  ylim(-1.5, 7)  +  # Set y-axis limits
  scale_alpha_manual(values = c("Coastal" = 1, "Non-Coastal" = 0.5)) +  # Adjust transparency level
  geom_text(aes(label = town), hjust = 1.1, vjust = 1.1, check_overlap = TRUE, size = 3) +

  labs(x = "Difference in Republican Vote Share", 
       y = "Difference in Democratic Vote Share", 
       title = "Difference in Vote Share by Town in Connecticut") 
```
