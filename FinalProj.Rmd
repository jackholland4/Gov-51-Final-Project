...
census_damage <- merge(damage, census, by = "GEOID", all = TRUE)
...
census_damage_zcta5 <- merge(geoid_zcta5, census_damage, by = "GEOID", all = TRUE)
...
census_damage_zcta5_town <- merge(zcta5_town, census_damage_zcta5, by = "ZCTA5", all = TRUE)

census_damage_zcta5_town <- census_damage_zcta5_town |>
  select(Town, Total.Damaged..any.type., Total.Housing.Units, 
         CVAP_TOT10, CVAP_NHS10, CVAP_AIA10, CVAP_ASN10, CVAP_BLK10, CVAP_NHP10, CVAP_WHT10, CVAP_AIW10,
         CVAP_ASW10, CVAP_BLW10, CVAP_AIB10) 

treatmentdata <- aggregate(.~ Town, data = census_damage_zcta5_town, sum)

treatmentdata <- treatmentdata |>
  rename(town = Town)

finaldata <- merge(CT_votes, treatmentdata, by = "town", all = TRUE)
```
